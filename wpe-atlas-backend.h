/*
 * wpe-atlas-backend — custom libwpe backend bridging WPE WebKit into the Atlas BrowserServer.
 * Public API used by the BrowserServer (UIProcess). See ARCHITECTURE.md.
 */
#ifndef WPE_ATLAS_BACKEND_H
#define WPE_ATLAS_BACKEND_H
...
#define WPE_ATLAS_EXPORT __attribute__((visibility("default")))
 ...
 */
typedef void (*wpe_atlas_frame_handler)(void* userdata,
                                       const uint8_t* argb,
                                       uint32_t width, uint32_t height,
                                       uint32_t stride);

/*
 * Create a wpe_view_backend bound to an Atlas frame sink. Pass the result to
 ...
 * scaledWidth×scaledHeight before readback, so the frame the handler receives (and thus the Atlas
...
WPE_ATLAS_EXPORT
void wpe_atlas_view_backend_resize(struct wpe_view_backend*, uint32_t width, uint32_t height);

/* Visibility: forwards to WebKit so it can throttle rendering when the card is backgrounded. */
WPE_ATLAS_EXPORT
void wpe_atlas_view_backend_set_visible(struct wpe_view_backend*, bool visible);
